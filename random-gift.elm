    ...
    case msg of
        ...
        FileDataSucceed str ->
            let
                ...
                emailList =
                    List.filter (\email -> email /= "") (List.map extractEmail parsedList.records)

                newMessage =
                    (Basics.toString (List.length emailList)) ++ "개의 이메일이 추가되었습니다 :)"
            in
        ...
        UpdateWinner giftName idx nextCmd winnerIdx ->
            let
                winnerEmail =
                    takeEmailFromList model.emailList winnerIdx

                newEmailList =
                    emailListWithout model.emailList winnerIdx
            in
                { model
...
updateWinnerCmdWrapper name maxIdx count =
    if count == 0 then
        Task.perform identity identity (Task.succeed ResetItem)
        -- send Msg without running Task
    else
        (Random.generate
